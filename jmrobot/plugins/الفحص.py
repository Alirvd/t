...
from jmrobot import StartTime, jmrobot, jmthonversion
...
**{EMOJI} اصدار العلويه:** `{telever}`
**{EMOJI} اصدار النبي :** `{jmver}`
**{EMOJI} اصدار للعلويه :** `{pyver}`
**{EMOJI} الوقت :** `{uptime}`
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس العلويه يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار العلويه :** `{jmthonversion}`\n"
    jmthon_caption += f"**{EMOJI} اصدار العلويه :** `{python_version()}\n`"
    jmthon_caption += f"**{EMOJI} المالك:** {mention}\n"
    return jmthon_caption


@jmrobot.ar_cmd(pattern="السورس$")
async def repo(event):
    RR7PP = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await jmrobot.inline_query(RR7PP, "السورس")
...
ROZ_T = (
    f"**⌯︙بوت العلويه تعمل بنجاح 🤍،**\n"
    f"**   - اصدار التليثون :** `1.23.0\n`"
    f"**   - اصدار العلويه :** `4.0.0`\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        await jmrobot.get_me()
        if query.startswith("السورس") and event.query.user_id == jmrobot.uid:
            buttons = [
                [
